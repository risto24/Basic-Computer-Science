...
GPUとは「Graphics Processing Unit」の略で、3Dグラフィックスなどの画像描写を行う際に必要となる計算処理を行う半導体チップ(プロセッサ)のことである

マシンにおける頭脳といえば上記で説明したCPUをイメージするが、GPUもまた大きな役割を担っている。
...
例えば、高品質の映像（ゲーム等）を出力する際には、光の角度などといった、様々な影響を緻密に計算して、より現実に近い品質で描写知なければならない為、膨大な並列計算処理が必要になり、このような計算処理もCPUに任せるのは効率が悪いといえるだろう。

そのため、描写計算処理は並列処理を得意とするGPUに仕事を任せてしまうのが一般的である。

このように、GPUは映像を描写するような定型的かつ膨大な計算処理を行うのに適したプロセッサであり、HDDからプログラムデータを読み出し、メモリへロードして実行するといった複雑な処理はCPUの方が得意である。

そして、この両者の差をあらわすのがコア数である。

CPUでは1つのプロセッサ上に複数個しか搭載していないのが一般的であるのに対し、GPUは桁違いの数千個搭載しているのが一般的である。

**CPUが全体の処理を行う司令官だとすると、GPUは定型化された大量の処理を大人数ですばやく行う工場のように例えることができる。**

<br>

## GPGPU (General-purpose computing on graphics processing units)
上記のように元来GPUは日々進化してきており、2D描写から3D描写などへ変化、さらに光の角度を計算した陰影など、より現実に近い品質を出力するにつれて、GPU自身の持つパワーが有り余るようになってきている。

一般的な書類作成やプレゼンテーションなどではGPUはほんの数％しか機能していない状態である。

その結果、GPUにはGPGPU（General-Purpose computing on GPU）という技術が登場し、画像処理だけでなくCPUのような汎用処理も行うようになるのである。

これによってCPUの負荷削減や、条件が揃えばCPUよりも格段に高速処理できるようになったのである。

たとえば、機械学習や、暗号解読、データベース処理、気候や天体などのシミュレーションにも多く使用されている。

このようにGPUは様々な並列処理に使用されており、GPUで構築したサーバーはスーパーコンピューターよりも遥かに安価に作成できることや、分野によっては速度を上回る事から、今後も需要は高まっていくと言えるだろう。
...
**SRAMにはL1キャッシュとL2キャッシュ等があり、それぞれが必要な役割を担っている。** <br>
※CPUによってはL4キャッシュまでもっているものもある。
